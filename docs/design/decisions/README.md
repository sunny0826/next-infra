...
本目录冻结进入 Goal 1 前必须明确的工具链、Desktop 生命周期、Bridge 发布边界与签名/发布边界。五项决策共同生效；摘要不能替代各决策正文。
...
| [DEC-G1-04](./DEC-G1-04-keychain-signing.md) | Partially Superseded | Keychain 方向已取消（2026-08-07 用户决策）；签名/发布边界部分仍有效；Secrets 存 SQLite `connection_secrets` |
...
- 本地 Goal 1 使用 Mock/Fixture，不安装 Agent 集成，也不修改外部基础设施，因此可以开始。
- Goal 1 ad-hoc Mock/Fixture 构建使用占位 bundle ID `dev.guoxudong.next-infra.dev`；它只是 bootstrap identity，不是 Apple Development identity，也不得用于发布或可信 MCP 自动拉起。
- **Keychain 方向已取消（2026-08-07 用户决策）**：Secrets 一律存 SQLite `connection_secrets`；不追求 Keychain 迁移。
- 正式 release bundle ID、Apple Team、Developer ID certificate/profile 与公证凭据仍待用户提供。缺少这些条件只阻塞发布门和可信自动拉起验收，不阻塞本地骨架。
...
实现若需要改变精确版本、crate 依赖方向、启动/退出语义、Bridge 路径或协议窗口、签名/更新策略，必须先修改对应决策并完成一次新的独立 Review，不能在功能提交中顺带改变。

> **注（2026-08-07）：** Keychain 方向已取消；Secret 一律存 SQLite `connection_secrets`，不再有 Keychain 身份变更。
