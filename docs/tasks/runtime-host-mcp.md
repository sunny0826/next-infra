...
- **状态：** `REVIEW / LOCKED-CONSOLE-LIFECYCLE-SMOKE-DEFERRED / LIVE-SIGNATURE-DEFERRED-BY-USER`。
...
- **阶段证据（2026-08-05）：** Desktop→UDS→Bridge→MCP real smoke 已通过；真实 Apple Event Quit 写 marker、移除 socket，随后新 Bridge保持 suppressed。Codex CLI 使用 ephemeral/ignore-user-config/inline MCP config 发起验收，但被账户 usage limit 在 tool invocation 前阻止；Hermes 未安装。用户随后明确暂缓 MCP Agent 与 Apple signing identity 验收，并授权继续 Goal 5；GATE-G4 因此外部验收保持 deferred 而非 `PASS`，锁屏 close→hide/restore 证据同样未伪造通过。
