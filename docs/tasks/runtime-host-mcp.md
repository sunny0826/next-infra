...
- **阶段证据（2026-08-05）：** Desktop→UDS→Bridge→MCP real smoke 已通过；真实 Apple Event Quit 写 marker、移除 socket，随后新 Bridge 保持 suppressed。Codex CLI 使用 ephemeral/ignore-user-config/inline MCP config 发起验收，但被账户 usage limit 在 tool invocation 前阻止；Hermes 未安装。GATE-G4 因真实 Codex、live signed auto-launch 与锁屏 close→hide/restore 证据缺失保持 `BLOCKED-EXTERNAL`。
