...
- **独占路径：** 根 manifests/lockfiles/toolchain 配置、所有初始 crate/app manifests；本任务是 Goal 1 Bootstrap Captain。
- **只读输入：** crate 拓扑、版本矩阵、Bridge 安装和签名决策。
- **范围：** 空 crates/targets、工具链固定、依赖方向 guard、最小 lint/test/build 命令。
- **非目标：** 不创建数据库表、真实 Connector、MCP Tool、业务页面或 Secret。
- **输出：** 可构建 workspace、独立 Core 测试目标、独立 `next-infra-mcp` 二进制目标、后续 owner 可认领的空目录。
- **验收：** Core 不依赖 Tauri；Bridge 不成为 Desktop 默认 binary；无 Provider SDK；版本不依赖全局漂移。
- **验证：** `rtk cargo metadata --format-version=1`、`rtk cargo fmt --all --check`、`rtk cargo clippy --workspace --all-targets -- -D warnings`、`rtk cargo test --workspace`、Desktop build/tauri build。
